...
def add_user(environ, user: User) -> dict[str, Any]:
    ...
    if not name or not password:
        return {'error': 'Query parameters "name" and "password" must be provided.', 'status': '405 Method Not Allowed'}

    username = name[0].replace('\'', '').lower()

    user_data = user.user_data

    if not any(d.get('username') == username for d in user_data):
